...
#### Where it is: 
In MonsterBehavior script, we created enum NpcState that holds and switches between state of patroling or chasing. In response to state switch, ExecutePatrol() or ExecuteChase() will be called. 
...
- dialogue mode (IsTalking)
- setting mode (IsSettingsOpen)

#### Why it was useful here:
During dialogue or setting mode, multiple other systems needed to obey the same pause rules in response, including:
